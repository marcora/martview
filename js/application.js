...
Martview.Application = function (params) {
  ...
  var selectForm = function (menu_item) {
    ...
    query_bbar.items.each(function (item) {
      item.enable();
    });
    ...
    header.get('home_sep').show();
    header.get('mart').setText(Ext.util.Format.ellipsis(menu_item.mart_display_name, 20, true)).setTooltip(menu_item.mart_display_name).setHandler(function () {
      window.location.search = '?server=' + menu_item.mart_name;
    }).show();
    header.get('mart_sep').show();
    header.get('dataset').setText(Ext.util.Format.ellipsis(menu_item.dataset_display_name, 30, true)).setTooltip(menu_item.dataset_display_name).setHandler(function () {
  ...
  // event listeners
  var query_bbar = query.getBottomToolbar();
  query_bbar.items.get('submit').on('click', function () {
  ...
  var query_tbar = query.getTopToolbar();
  query_tbar.items.get('customize').on('click', function () {
    filters.show();
  });
