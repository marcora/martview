...
Martview.Results = Ext.extend(Ext.Panel, {
  ...
  load: function (data, format) {
    ...
    if (format == 'tabular') {
      ...
      var tpl = new Ext.XTemplate('<tpl for=".">', '<table class="item" style="margin-bottom: 5px;">', '<tr class="title">', '<td style="width: 50px; align: right; vertical-align: top;">{pdb_id} <img src="./ico/arrow-000-small.png" style="vertical-align: middle;" /></td><td>{title}</td>', '</tr>', '<tr>', '<td style="width: 50px; align: center; vertical-align: top;"><img style="width: 50px;" src="http://www.rcsb.org/pdb/images/{pdb_id}_asym_r_250.jpg" /></td>', '<td>', '<div class="attribute">Experiment type: {experiment_type}</div>', '<div class="attribute">Resolution: {resolution}</div>', '<div class="attribute">Space group: {space_group}</div>', '<div class="attribute">R work: {r_work}</div>', '</td>', '</tr>', '</table>', '</tpl>', '<div class="x-clear"></div>');
